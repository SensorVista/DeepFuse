...
TEST_F(PoolingLayerTest, MaxPoolingForward) {
    ...
    PoolingLayer<float> layer(PoolingType::Max, 2, 2, true);
...
TEST_F(PoolingLayerTest, AveragePoolingForward) {
    ...
    PoolingLayer<float> layer(PoolingType::Average, 2, 2, true);
...
TEST_F(PoolingLayerTest, MaxDeterministicPooling) {
    ...
    tensor<float> input = create_sequential_tensor({2, 2, 4, 4});
    PoolingLayer<float> layer(PoolingType::MaxDeterministic, 2, 2, true);
    
    // Perform forward pass
    tensor<float> output1 = layer.forward(input);
    EXPECT_EQ(output1.shape(), std::vector<int>({2, 2, 2, 2}));
    
    // Verify deterministic results
    tensor<float> output2 = layer.forward(input);
    EXPECT_TRUE(compare_tensors(output1, output2));
}
#endif
