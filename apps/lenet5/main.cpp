...
void train(
    dnn::TrainingModel<QTYPE>* network, 
    ...
    for (int epoch = 0; epoch < num_epochs; ++epoch) {
        ...
        for (int i = 0; i < num_train_images; i += batch_size) {
            ...
            dnn::tensor<QTYPE> input({ current_batch_size, channels, input_dim, input_dim });
            dnn::tensor<QTYPE> target({ current_batch_size, output_dim });

            // Convert batch data to float
            std::vector<QTYPE> input_data(current_batch_size * input_dim * input_dim);
            std::vector<QTYPE> target_data(current_batch_size * output_dim, static_cast<QTYPE>(0.0f));

            for (int b = 0; b < current_batch_size; ++b) {
                for (int j = 0; j < input_dim * input_dim; ++j) {
                    float norm = (static_cast<float>(train_images[indices[i + b]][j]) - 127.5f) / 127.5f;
                    input_data[b * input_dim * input_dim + j] = static_cast<QTYPE>(norm);
                }
                
                int label = train_labels[indices[i + b]];
                if (label >= output_dim)
                    throw std::out_of_range("Label index exceeds output_dim");
                target_data[b * output_dim + label] = static_cast<QTYPE>(1.0f);
            }
        ...
        for (int i = 0; i < num_test_images; ++i) {
            dnn::tensor<QTYPE> input({ 1, 1, input_dim, input_dim });
            // Normalize test images similarly
            std::vector<QTYPE> single_test_image_data(input_dim * input_dim);
            for(int j = 0; j < input_dim * input_dim; ++j) {
                float norm = (static_cast<float>(test_images[i][j]) - 127.5f) / 127.5f;
                single_test_image_data[j] = static_cast<QTYPE>(norm);
            }
            input.upload(single_test_image_data.data());

            auto output = network->forward(input);
            std::vector<QTYPE> host_output(output.size());
        ...
        float loss_value = static_cast<float>(total_loss) / static_cast<float>(num_train_images);
        std::cout
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        dnn::LeNet5<QTYPE> network(0.01, 0.9, true);
