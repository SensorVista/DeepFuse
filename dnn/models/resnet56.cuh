...
/// ResNet-56 with 9 blocks per stage for deeper representation.
/// Common benchmark model for CIFAR-10/100 research.
/// Balances depth and compute for scalable residual learning.    

template<typename T>
class ResNet56 : public TrainingModel<T> {
public:
    explicit ResNet56(int num_classes = 10, T lr = 0.1, T momentum = 0.9) {
        const int C = 16;
        ...
        for (int i = 0; i < 9; ++i) this->layers_.push_back(std::make_unique<ResidualBlock<T>>(C, C, 1, false));
        this->layers_.push_back(std::make_unique<ResidualBlock<T>>(C, C * 2, 2, false));
        for (int i = 1; i < 9; ++i) this->layers_.push_back(std::make_unique<ResidualBlock<T>>(C * 2, C * 2, 1, false));
        this->layers_.push_back(std::make_unique<ResidualBlock<T>>(C * 2, C * 4, 2, false));
        for (int i = 1; i < 9; ++i) this->layers_.push_back(std::make_unique<ResidualBlock<T>>(C * 4, C * 4, 1, false));
