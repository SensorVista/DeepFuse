...
template<typename T>
void ResNet20<T>::save(const std::string& path) const {
...
template<typename T>
std::unique_ptr<ResNet20<T>> ResNet20<T>::load(const std::string& path, bool training_enabled) {
    ...
    auto model = std::make_unique<ResNet20<T>>(10, 0.1, 0.9, training_enabled);
    if (num_blocks1 != static_cast<int>(model->res_stage1_.size()) ||
        ...
        throw std::runtime_error("Block count mismatch in ResNet20::load");
...
template class ResNet20<float>;
// template class ResNet20<__half>;
