...
template<typename T>
void ResNet56<T>::save(const std::string& path) const {
...
template<typename T>
std::unique_ptr<ResNet56<T>> ResNet56<T>::load(const std::string& path, bool training_enabled) {
    ...
    auto model = std::make_unique<ResNet56<T>>(10, 0.1, 0.9, training_enabled);
    if (num_blocks1 != static_cast<int>(model->res_stage1_.size()) ||
        ...
        throw std::runtime_error("Block count mismatch in ResNet56::load");
...
template class ResNet56<float>;
// template class ResNet56<__half>;
