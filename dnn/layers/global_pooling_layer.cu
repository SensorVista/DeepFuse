...
template<typename T>
GlobalPoolingLayer<T>::GlobalPoolingLayer(PoolingType type, bool training_enabled)
...
template<typename T>
tensor<T> GlobalPoolingLayer<T>::backward(const tensor<T>& grad_output) {
    if (!this->training_enabled_ || !input_cache_) {
        throw std::runtime_error("GlobalPoolingLayer::backward requires cached input in training mode.");
    }
    const tensor<T>& input = *input_cache_;
    const auto& shape = input.shape();
    const int B = shape[0];
    const int C = shape[1];
    const int H = shape[2];
    ...
    utils::CHECK_CUDNN_EX(cudnnPoolingBackward(
        ...
        &alpha,
        output_desc_,
        nullptr, // y (not used in pooling backward)
        output_desc_,
        grad_output.data(),
        input_desc_,
        input.data(),
...
template<typename T>
void GlobalPoolingLayer<T>::save(std::ostream& out) const {
    int type_int = static_cast<int>(type_);
    out.write(reinterpret_cast<const char*>(&type_int), sizeof(type_int));
}

template<typename T>
void GlobalPoolingLayer<T>::load(std::istream& in) {
    int type_int;
