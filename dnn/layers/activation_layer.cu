...
template<typename T>
ActivationLayer<T>::ActivationLayer(ActivationType type, bool training_enabled) 
    : Layer<T>(training_enabled), type_(type) {
#ifdef ENABLE_CUDNN
...
template<typename T>
std::string ActivationLayer<T>::name() const { return std::string(activation_type_to_string(type_)); }
...
tensor<T> ActivationLayer<T>::backward(const tensor<T>& grad_output) {
    ...
    if (this->training_enabled_) {
        if (!input_cache_) throw std::runtime_error("ActivationLayer: input_cache_ not set for backward");
        used_input = &(*input_cache_);
    } else {
        throw std::runtime_error("ActivationLayer: backward called without input argument in stateless mode");
    }
    ...
    utils::CHECK_CUDNN_EX(cudnnActivationBackward(handle, act_desc_, &alpha,
        ...
        used_input->desc(), used_input->data(), &beta, grad_input.desc(), grad_input.data()));
#else
    const int BLOCK_SIZE = 256;
    int num_blocks = (size + BLOCK_SIZE - 1) / BLOCK_SIZE;
    ...
    switch (type_) {
        case ActivationType::ReLU:
            relu_backward_kernel<<<num_blocks, BLOCK_SIZE>>>(grad_output.data(), used_input->data(), grad_input.data(), size);
            break;
        case ActivationType::Sigmoid:
            sigmoid_derivative_kernel<<<num_blocks, BLOCK_SIZE>>>(grad_output.data(), used_input->data(), grad_input.data(), size);
            break;
        case ActivationType::Tanh:
            tanh_backward_kernel<<<num_blocks, BLOCK_SIZE>>>(grad_input.data(), grad_output.data(), used_input->data(), size);
            break;
        case ActivationType::GELU:
            gelu_backward_kernel<<<num_blocks, BLOCK_SIZE>>>(grad_output.data(), used_input->data(), grad_input.data(), size);
            break;
        default:
            throw std::runtime_error("Unsupported activation type");
    }
