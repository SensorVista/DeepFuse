...
ConvLayer<T>::ConvLayer(
    ...
    int stride, 
    int padding, 
    const std::vector<std::vector<bool>>& connection_table,
...
template<typename T>
tensor<T> ConvLayer<T>::backward(const tensor<T>& grad_output) {
    const tensor<T>* used_input = nullptr;
    if (this->training_enabled_) {
        if (!input_cache_) throw std::runtime_error("ConvLayer: input_cache_ not set for backward");
        used_input = &(*input_cache_);
    } else {
        throw std::runtime_error("ConvLayer: backward called without input argument in stateless mode");
    }
    const int N = used_input->shape()[0];
    const int C_in = used_input->shape()[1];
    const int H_in = used_input->shape()[2];
    const int W_in = used_input->shape()[3];
    ...
    utils::CHECK_CUDNN_EX(cudnnGetConvolutionBackwardFilterAlgorithm(
        handle_dnn, used_input->desc(), grad_output.desc(), conv_desc_, filter_desc_,
    ...
    utils::CHECK_CUDNN_EX(cudnnConvolutionBackwardFilter(
        handle_dnn,
        &alpha, used_input->desc(), used_input->data(),
        grad_output.desc(), grad_output.data(),
    ...
    // Grad weights
    {
        ...
        conv_weight_grad_2d<T><<<blocks, threads>>>(
            used_input->data(), grad_output.data(), this->grad_weights_.data(),
            use_sparse_connectivity_ ? connection_mask_dev_.data() : nullptr,
            N, C_in, C_out, H_in, W_in, H_out, W_out, K, stride_, padding_);
    }
...
template<typename T>
void ConvLayer<T>::save(std::ostream& out) const {
    ...
    int kernel_size_len = static_cast<int>(kernel_size_.size());
    out.write(reinterpret_cast<const char*>(&kernel_size_len), sizeof(kernel_size_len));
    out.write(reinterpret_cast<const char*>(kernel_size_.data()), sizeof(int) * kernel_size_len);
    ...
    if (use_sparse_connectivity_) {
        ...
        connection_mask_dev_.download(mask.data());
        out.write(reinterpret_cast<const char*>(mask.data()), mask_size);
    }
}

template<typename T>
void ConvLayer<T>::load(std::istream& in) {
    ...
    int kernel_size_len;
    in.read(reinterpret_cast<char*>(&kernel_size_len), sizeof(kernel_size_len));
    kernel_size_.resize(kernel_size_len);
    in.read(reinterpret_cast<char*>(kernel_size_.data()), sizeof(int) * kernel_size_len);
    ...
    if (use_sparse_connectivity_) {
        int mask_size = out_channels_ * in_channels_;
        std::vector<uint8_t> mask(mask_size);
        in.read(reinterpret_cast<char*>(mask.data()), mask_size);
        connection_mask_dev_.upload(mask.data());
    }
}
